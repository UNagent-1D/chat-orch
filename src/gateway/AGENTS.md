...
| `acr_client.rs` | Agent Config Registry (Go) | `GET /api/v1/tenants/:id/profiles/:pid/configs/active` |

## Caches (moka — in-memory, read-only)

| Cache | Key | TTL | Max Entries | Purpose |
|-------|-----|-----|-------------|---------|
| `channel_cache.rs` | `{type}:{channel_key}` | 5 min | 100K | Channel → Tenant resolution |
| `config_cache.rs` | `{tenant_id}:{profile_id}` | 2 min | 50K | Agent config (LLM params, tools) |

Both caches use `moka::future::Cache::try_get_with()` for thundering-herd
