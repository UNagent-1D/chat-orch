    ...
    fn into_response(self) -> Response {
        let (status, message) = match &self {
            ...
            AppError::SessionCreation(_) => {
                (StatusCode::INTERNAL_SERVER_ERROR, "session creation failed".into())
            }
            ...
            AppError::Downstream(_) => {
                (StatusCode::BAD_GATEWAY, "downstream service unavailable".into())
            }
            AppError::LlmError(_) => {
                (StatusCode::BAD_GATEWAY, "LLM provider error".into())
            }
            AppError::Redis(_) => {
